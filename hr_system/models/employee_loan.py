...
class EmployeeLoan(models.Model):
    ...
    def create(self, vals_list):
        ...
        for rec in res:
            ...
            if rec.employee_id.employee_basic_salary > 0:
                if rec.loan_amount * 100 / rec.employee_id.employee_basic_salary > 40:
                    raise ValidationError(
                        "Your loan limit is 40% of your basic salary")
                if len(self.env['employee.loan'].search([('employee_id', '=', rec.employee_id.id), ('loan_state', 'in', ['approved', 'pending'])])) > 1:
                    raise ValidationError(
                        "Your already have unpaid loan")
                rec.remaining_amount = rec.loan_amount
            else:
                raise ValidationError(
                    "Please contact with your hr to edit your basic salary from 0")
        return res
