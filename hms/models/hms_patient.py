...
class HmsPatient(models.Model):
    ...
    _sql_constraints = [('unique_number', 'unique("phone_number")',
        ...
        for rec in self:
            if len(rec.logs_ids) > 1:
                raise ValidationError(
                    f"{rec.full_name} has logs records, so you can't delete them, just archive them"
                )
            else:
                res = super().unlink()
        return res
